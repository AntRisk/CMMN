...
class TimeLSTM3Cell(RNNCell):
    ...
    def __init__(self,
                 ...
                 is_ut=False,
                 **kwargs):
        super(TimeLSTM3Cell, self).__init__(_reuse=reuse, name=name, dtype=dtype, **kwargs)
    ...
    def get_config(self):
        ...
        base_config = super(TimeLSTM3Cell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


def TimeLSTM3(x, is_ut=False):
    rnn_cell = TimeLSTM3Cell(10, is_ut=is_ut)
...
def target_net(data, num_units):
    ...
    for d in data:
        ...
        c_m_tilde = (1 - i_m * t_m1) * c + i_m * t_m1 * np.tanh(np.matmul(x, w_xc) + np.matmul(h, w_hc) + b_c)
        c_m = (1 - i_m) * c + i_m * t_m2 * np.tanh(np.matmul(x, w_xc) + np.matmul(h, w_hc) + b_c)
...
def ut():
    ...
    output, states = TimeLSTM3(X, True)
